...
Extrai campos específicos do PDF sem demora
"""
...
class ContratoExtractorRapido:
    ...
    def _extrair_campos(self):
        ...
        if data:
            ...
            print(f"[ERRO] Data primeira prestacao: NAO ENCONTRADA")
        ...
        if prestacao:
            ...
            print(f"[ERRO] Prestacao total: NAO ENCONTRADA")
        ...
        if prazo:
            ...
            print(f"[ERRO] Prazo: NAO ENCONTRADO")
        ...
        if tx:
            ...
            print(f"[ERRO] Taxa juros: NAO ENCONTRADA")
        ...
        if prest_base:
            ...
            print(f"[ERRO] Prestacao base: NAO ENCONTRADA")
    ...
    def _extrair_prazo(self):
        ...
        patterns = [
            r'([0-9]{2,3})\s*(?:meses?|m)\s*(?:de\s+(?:prazo|amortização)|referentes?)',
    ...
    def _extrair_prestacao_base(self):
        ...
        patterns = [
            r'(?:prestação|prest)\s+(?:base|mensal).*?(?:Cr\$)?\s*([0-9]{1,3}[.,][0-9]{2,6})',
        ]
    ...
    def validar(self, codigo):
        ...
        print("=" * 100)
        print("VALIDAÇÃO CONTRA BANCO")
        print("=" * 100)
        
        try:
            c = Contrato.objects.get(codigo=codigo)
        except:
            print(f"❌ Contrato {codigo} não encontrado")
            return False
        ...
        if 'prestacao_inicial' in self.fields_extracted:
            ...
            if db_val:
                ...
                match = diff_pct < 1
                icon = "✅" if match else "❌"
                print(f"{icon} Prestação: OCR={ocr_val:.2f} | DB={db_val:.2f} (diff={diff_pct:.2f}%)")
                if not match:
                    sucesso = False
...
def main():
    ...
    if extrator.extrair():
        ...
        if extrator.validar("1234"):
            print("[OK] VALIDACAO OK - EXTRACAO FUNCIONANDO!")
        else:
            print("[ERRO] Ha divergencias - verificar patterns")
