...
csv_path = r'C:\Users\fabri\cofluhab\exports\contrato_mutuario_map_v2.csv'
...
# Filter high confidence (score >= 0.9)
...
# Insert high confidence mappings
print(f"\nInserindo {len(high_conf)} mapeamentos de alta confiança...")
...
# Check mutuario 1 (codigo 000327)
print("\nVerificando mutuario_id=1 (codigo 000327):")
cur.execute("SELECT * FROM contrato_mutuario_map WHERE mutuario_id=1")
maps = cur.fetchall()
print(f"  Encontrados: {len(maps)} mapeamentos")
for row in maps[:3]:
    print(f"    contrato={row[0]} → mutuario={row[1]} (score={row[2]}, {row[3]})")
